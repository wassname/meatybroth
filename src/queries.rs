...
// Same reader ranking definitions as meatybroth/ranking.py; bounded SQL pages. -- Pi/gpt-6-astra
const ELIGIBLE: &str = "WITH eligible AS (
 SELECT p.* FROM posts p WHERE p.created_at BETWEEN :since AND :until
 AND NOT EXISTS (SELECT 1 FROM policy_exclusions x WHERE x.event_id=p.source_id)
 AND p.author_id NOT IN (SELECT m.value FROM moderation_lists l,json_each(l.members_json) m
   WHERE l.identifier='nsfw' AND m.type='text')
)";
const CARD_DEFAULTS: &str = "0 AS n_reply_authors,0 AS n_replies,0 AS latest_activity,
...
pub fn feed(db: &Connection, search: &Search, root: &str, now: i64) -> Result<Vec<Post>, Error> {
    let matches = if search.expression.is_some() {
        "SELECT p.canonical_id,bm25(posts_fts) AS bm25 FROM posts_fts
         JOIN posts p ON p.rowid=posts_fts.rowid WHERE posts_fts MATCH :expression"
    } else {
    ...
    let sql = match search.mode.as_str() {
        "conversations" => format!("{prefix}, members AS (
          ...
          SELECT m.thread,COUNT(DISTINCT CASE WHEN m.created_at>=:until-86400
            AND m.canonical_id!=m.thread AND (r.author_id IS NULL OR m.author_id!=r.author_id)
            THEN m.source||':'||m.author_id END) AS n_reply_authors,
            COUNT(*)-MAX(m.canonical_id=m.thread) AS n_replies,
            MAX(m.created_at) AS latest_activity,MAX(m.canonical_id=m.thread) AS root_present
          FROM members m LEFT JOIN eligible r ON r.canonical_id=m.thread
            AND coalesce(r.root_id,r.canonical_id)=m.thread GROUP BY m.thread
        ), cards AS (
          SELECT m.*,x.bm25,ROW_NUMBER() OVER (PARTITION BY m.thread ORDER BY
            ...
            m.canonical_id ASC) AS position
          FROM members m JOIN matches x USING(canonical_id)
        ...
        endpoint_edges AS MATERIALIZED (
          ...
          JOIN reach r ON r.author=p.author_id WHERE r.hops<=:reach
          ORDER BY {}p.created_at DESC,p.canonical_id ASC",
          if search.order=="connections" { "r.mass DESC," } else { "" }),
        _ => format!("{prefix} SELECT p.*,x.bm25,{CARD_DEFAULTS} FROM eligible p
          JOIN matches x USING(canonical_id) ORDER BY {}p.created_at DESC,p.canonical_id ASC",
          if search.mode=="relevance" { "x.bm25 ASC," } else { "" }),
    };
    let mut stmt = db.prepare(&format!("{sql} LIMIT :limit OFFSET :offset"))?;
    let params = named_params! {
        ":since": now-WINDOW, ":until": search.before.unwrap_or(now),
        ":expression": search.expression, ":limit": (PAGE_SIZE+1) as i64,
...
pub fn get(db: &Connection, id: &str, now: i64) -> Result<Option<Post>, Error> {
    let mut stmt = db.prepare(&format!(
        "{ELIGIBLE} SELECT *,NULL AS bm25,{CARD_DEFAULTS}
        FROM eligible WHERE canonical_id=:id"
    ))?;
...
pub fn count(db: &Connection, now: i64, parent: Option<&str>) -> Result<i64, Error> {
    ...
    Ok(db.query_row(
        &format!("{ELIGIBLE} SELECT count(*) FROM eligible WHERE {condition}"),
...
pub fn children(db: &Connection, id: &str, now: i64) -> Result<Vec<Post>, Error> {
    let mut stmt = db.prepare(&format!(
        "{ELIGIBLE} SELECT *,NULL AS bm25,{CARD_DEFAULTS}
        FROM eligible WHERE parent_id=:id ORDER BY created_at,canonical_id"
    ))?;
    let rows = stmt
        .query_map(
