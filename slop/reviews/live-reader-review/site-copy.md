...
### Final exact-ID state validation — 2026-09-14

With the owner-provided release database `/tmp/meatybroth-dbscan-release.sqlite`, I compared actual local8088 card IDs against membership tables, not only checkbox state or card count. DBSCAN default returned 100 cards and all 100 have non-noise `post_dbscan_topics` membership. DBSCAN `unsorted=true` returned 100 cards and all 100 have `topic_id = -1`. K-means `topics=1&topics=0` returned 100 cards and all 100 belong to topic 1 or 0. Thus default, explicit Unsorted, and multi-topic union filtering pass on this frozen release DB.

A real browser K-means URL with two topics and page two changed Group by to DBSCAN. The resulting URL was `?mode=topics&clustering=dbscan&q=&embedding=titan`: it had no selected topics, Unsorted, or page parameter, and rendered 100 cards. Algorithm-switch reset therefore passes.

The remaining model-switch gap later passed on the same release DB. A browser started at Titan/K-means with `topics=1&topics=0&page=2`; changing Model to MiniLM produced `?mode=topics&clustering=kmeans&q=&embedding=minilm`, with no topics, Unsorted, or page. MiniLM was visibly selected and all 100 returned IDs have MiniLM `post_topics` membership. That request was a Topics GET with no query text, and the route only reads topic membership; it initiated no provider call. This does not claim that unrelated background writer calls were absent. No public or AWS action occurred.
