...
A real browser K-means URL with two topics and page two changed Group by to DBSCAN. The resulting URL was `?mode=topics&clustering=dbscan&q=&embedding=titan`: it had no selected topics, Unsorted, or page parameter, and rendered 100 cards. Algorithm-switch reset therefore passes. This local runtime renders only Titan, so an actual MiniLM selector change could not be exercised; its source has the same `onchange` reset logic, but that is source evidence rather than a MiniLM UI test. No public or AWS action occurred.
