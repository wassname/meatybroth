...
Three small defects were returned to the app owner: Similar appears as a `View` select option on its error page even though it must remain a post action; zero-card error pages still show pager/date controls; and Status summary uses `bedrock`/`minilm` rather than plain `Titan`/`MiniLM` while model details are already available below. A source audit then found a state bug: `Search::parse` and `query_string` retain numeric `topics` when Model or Group by changes, and `topic_events` applies them to the new space/table. Model/algorithm changes must clear topics and Unsorted and reset pagination before submit; selection is not accepted until that is fixed and ID membership is tested. No public traffic or auth action occurred.
